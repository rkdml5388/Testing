...
x_train = np.vstack((x1_train, x2_train,x2_train_ord2, x3_train,x3_train_ord2,x3_train_ord3))
x_test = np.vstack((x1_test, x2_test,x2_test_ord2, x3_test,x3_test_ord2,x3_test_ord3)).transpose()
x_train = x_train.transpose()
y_train = y_train.transpose()
print(x_test)

X_train = tf.placeholder(tf.float32, shape=[None, 6])
Y_train = tf.placeholder(tf.float32, shape=[None, 1])

W = tf.Variable(tf.random_normal([6, 1]), name='weight1')
